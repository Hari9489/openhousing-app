...
Docker
Created a Dockerfile to package the FastAPI application, trained model, and dependencies into a single container.
Configured the application to run with Uvicorn on port 8001.
Ensured the API, static web interface, and ML model are all available from the same container.

Run locally with Docker:

docker build -t openhousing .
docker run -p 8001:8001 openhousing
...
Deployed the Docker container to Render as a Web Service.
Configured Render to build the Docker image automatically from the repository.
Exposed port 8001 for serving the FastAPI application.
Enabled automatic deployments on new commits, allowing updates to be published without manual server configuration.
